# Hi, I’m Rob 👋

Frontend Engineer | React & TypeScript | UI-Focused Problem Solver

I’m a frontend-savvy software engineer living in New Jersey with experience developing modern, scalable web apps using React, TypeScript, and JavaScript.

I have expertise in designing clean, maintainable UI architectures, component systems, and user experiences, often in environments where performance, accessibility, and reliability are paramount. My experience also includes low-level software development in C/C++, which provides me a solid grounding in performance-conscious thinking and system design.

I find it rewarding to connect engineering excellence with well-crafted user interfaces and am particularly interested in frontend engineering opportunities where product quality, code expressiveness, and developer experience are important.

## 💻 Technical Focus

### Frontend

- React
- TypeScript & JavaScript (ES6+)
- Responsive UI & reusable design systems
- State management & controlled components
- Accessibility-minded UI development
  
### Additional Experience
- Node.js
- REST APIs
- MongoDB
- C / C++ (performance-oriented development)
- Python

## Connect With Me
🔗 [Connect with me on LinkedIn](https://www.linkedin.com/in/rob-curcio-a4097a291/)

## What I’m Looking For
### I’m currently seeking frontend or frontend-leaning full-stack roles where I can:
- Build high-quality user interfaces
- Work deeply with React & TypeScript
- Collaborate with product and design teams
- Continue growing as a frontend and backend specialist

## Known Languages 🌐
I work with the following languages:

![HTML5](https://img.shields.io/badge/HTML5-%23E34F26.svg?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-%231572B6.svg?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/TypeScript-%23F7DF1E.svg?style=flat&logo=javascript&logoColor=black)
![JavaScript](https://img.shields.io/badge/JavaScript-%23F7DF1E.svg?style=flat&logo=javascript&logoColor=black)
![JavaScript](https://img.shields.io/badge/React-%23F7DF1E.svg?style=flat&logo=javascript&logoColor=black)
![JavaScript](https://img.shields.io/badge/Node-%23F7DF1E.svg?style=flat&logo=javascript&logoColor=black)
![Java](https://img.shields.io/badge/C-%23007396.svg?style=flat&logo=java&logoColor=white)
![Java](https://img.shields.io/badge/C++-%23007396.svg?style=flat&logo=java&logoColor=white)
![Python](https://img.shields.io/badge/Python-%233776AB.svg?style=flat&logo=python&logoColor=white)
